    ...
    <el-table v-loading="isShowLoading" :data="tableData" stripe row-key="id">
      ...
      <el-table-column label="操作" fixed="right" width="200px">
        <template slot-scope="scope">
          <el-button size="mini"  v-if="userRight" icon="el-icon-edit" @click="editFun(scope.$index, scope.row)">{{ userRight ?"编辑":"查看"}}</el-button>
  ...
  methods: {
    ...
    handleDelete(index, res) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提 示", {
          ...
          this.$myApi.http.post("/server/api/v1/staff/contract/delete", { id: res.id }).then(res => {
              this.reload();
    ...
    handleDeleteAll() {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提 示", {
          ...
          this.$myApi.http.post("/server/api/v1/staff/contract/deleteByStaffCode", { staffCode:this.staffInfo.code }).then(res => {
              this.reload();
              this.$message.success("删除成功！");
            });
        })
