...
export default {
  ...
  computed: {
    ...
    pageInfo(){
      return {
        reqParams:{//请求分页参数
            url:"/server/api/v1/staff/working/getAll",
  ...
  methods: {
    ...
    handleDelete(index, res) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提 示", {
          ...
          this.$myApi.http.post("/server/api/v1/staff/working/delete", { id: res.id }).then(res => {
              this.reload();
    ...
    handleDeleteAll() {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提 示", {
          ...
          this.$myApi.http.post("/server/api/v1/staff/working/deleteByStaffCode", { staffCode:this.staffInfo.code }).then(res => {
              this.reload();
              this.$message.success("删除成功！");
            });
        })
