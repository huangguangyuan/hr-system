...
export default {
  ...
  data() {
    return {
      ...
      curInfo: {}, //传值给子组件
      filter:{searchKey:'',searchField:['name','taxableTxt','description']}
    };
  },
  computed: {
    pageInfo(){
      return {
        reqParams:{
            url:"/server/api/v1/salaryItem/getAll",
            data:{}
          }
        }
    },
    ...
    tableData(){
      return this.pageList.map(item => {
        ...
        item.taxableTxt = taxableTxt(item.taxable);
        return item;
      });
    }
  },
  ...
  methods: {
    ...
    handleDelete(index, res) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提 示", {
          ...
          this.$myApi.http.post("/server/api/v1/salaryItem/delete", { id: res.id }).then(() => {
              this.reload();
              this.$message('取消成功');
            });
        })
