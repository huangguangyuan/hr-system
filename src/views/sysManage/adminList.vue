<template>
  <div class="wrap adminList">
    ...
    <el-table :data="queryTableDate" stripe row-key="id" border>
      ...
      <el-table-column label="操作" fixed="right" width="480px">
        <template slot-scope="scope">
          ...
          <el-button size="mini" icon="el-icon-edit-outline" @click='modifyPassWord(scope.$index, scope.row)'>修改密码</el-button>
          <el-button size="mini" icon="el-icon-plus" @click='addRole(scope.$index, scope.row)'>添加角色</el-button>
          <el-button size="mini" icon="el-icon-warning" @click='forbidden(scope.$index, scope.row)'>{{scope.row.status==1?'禁用':'启用'}}</el-button>
          <el-button
            size="mini"
            icon="el-icon-delete"
    ...
    <!-- 修改管理员 -->
    <el-dialog title="修改信息" :visible.sync="isShowModifyAdmin" :close-on-click-modal="false" width="65%">
      <modify-admin v-if="isShowModifyAdmin" v-on:listenIsShowAddAdmin="IsShowAddAdminFn" :modifyInfo='modifyInfo'></modify-admin>
    </el-dialog>
    <!-- 修改密码 -->
